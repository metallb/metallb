...
func (c *bgpController) ShouldAnnounce(l log.Logger, name string, _ []net.IP, pool *config.Pool, svc *v1.Service, epSlices []discovery.EndpointSlice, nodes map[string]*v1.Node) string {
	...
	if k8snodes.IsNetworkUnavailable(nodes[c.myNode]) {
		level.Warn(l).Log("event", "skipping should announce bgp", "service", name, "reason", "nodeNetworkUnavailable")
	...
	if k8snodes.MatchesExcludePattern(nodes[c.myNode], c.excludeNodeMetadata) {
		level.Warn(l).Log("event", "skipping should announce bgp", "service", name, "reason",
			fmt.Sprintf("exclude pattern %v", c.excludeNodeMetadata))
