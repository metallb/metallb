...
| Field | Description |
| --- | --- |
| `result` _[ConfigurationStateResult](#configurationstateresult)_ | Result indicates the configuration validation result. |
| `lastError` _string_ | LastError contains the error message from the last reconciliation failure.<br />This field is empty when Result is "Valid". |
