...
With this configuration, services labeled `environment: production` that receieved an IP from `first-pool` will be advertised
with `localPref=200`, while services labeled `environment: staging` will be advertised
with `localPref=100`.
