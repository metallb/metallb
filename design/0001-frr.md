...
simplify the deployment for end users as MetalLB will not need to manage a
...
speaker Pod, and the seperation of the FRR and speaker Pod lifecycles but
would add complexity to the end user.

Initially, control of the FRR container will be achieved by declarative control
of FRR through configuration and reloading of the FRR configuration file.
...
version but it will be possible to configure the version somehow (e.g. through
Helm).

You can restart speaker Pods for upgrades. However, Layer 2 memberlist code
...
* Extensibility options: FRR provides additional extensibility via its `dplane`
  ...
  providers. There does not appear to be equivalents in BIRD which make it less
  extensible. This extensibility would enable FRR to integrate with dataplanes
  other than the Linux kernel. For example, 1) hardware dataplanes 2) an Open
