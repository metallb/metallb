...
var _ = ginkgo.Describe("FRR BGP ExcludeNode IPV4", func() {
	var (
		...
		excludeCM     = "excludenodepattern" // configmap name is hardcoded to manifests
		cs            clientset.Interface
		svc           *corev1.Service
		testNamespace string
	)

	restartSpeakerPods := func() {
	...
	ginkgo.BeforeEach(func() {
      ...
      labels:
		...
		cm := map[string]string{"pattern.yaml": fmt.Sprintf(data, ann[0], ann[1])}

		cs = k8sclient.New()
		ginkgo.By("adding the excludenodepattern configmap")
