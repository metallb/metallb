...
var _ = ginkgo.Describe("ConfigurationStatus", func() {
	...
	ginkgo.It("should have condition frrk8sReconcilerValid true when valid FRRK8S-MODE config applied", func() {
		...
		// match `kubectl wait --for=condition=Ready configurationstatus/config-status -n metallb-system --timeout=30s`
		ginkgo.By("Waiting for Ready condition to be True")
		err = waitFor("Ready", metav1.ConditionTrue)
		Expect(err).NotTo(HaveOccurred())
		...
		Eventually(func() error {
			...
			for _, node := range allNodes.Items {
				...
				if err := checkCondition(want); err != nil {
					errs = append(errs, fmt.Errorf("condition check failed for %s: %w", want.Type, err))
				}
			}
	...
	ginkgo.It("should have condition frrk8sReconcilerValid false when FRRConfiguration conflicts with MetalLB FRRK8S-MODE config", func() {
		...
		wantCondition := metav1.Condition{
			Type:    "speaker-" + nodeName + "/frrk8sReconcilerValid",
		...
		Eventually(func() error {
			return checkCondition(wantCondition)
		}, 30*time.Second, 5*time.Second).Should(Succeed())
...
func waitFor(conditionType string, expectedStatus metav1.ConditionStatus) error {
...
func checkCondition(want metav1.Condition) error {
	...
	if err := k8sClient.Get(context.Background(), types.NamespacedName{
		...
		return fmt.Errorf("failed to get ConfigurationStatus: %w", err)
	}
