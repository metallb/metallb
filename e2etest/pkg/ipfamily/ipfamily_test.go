...
func TestIPFamilyForAddresses(t *testing.T) {
	tests := []struct {
		...
		{
			desc:   "required dual sack with ipv4 and ipv6 addresses",
			ips:    []string{"1.2.3.4", "100::1"},
			policy: v1.IPFamilyPolicyRequireDualStack,
			family: RequireDualStack,
		},
		{
			desc:   "prefer dual sack with ipv4 and ipv6 addresses",
			ips:    []string{"1.2.3.4", "100::1"},
			policy: v1.IPFamilyPolicyPreferDualStack,
			family: PreferDualStack,
		},
		{
			desc:   "prefer dual stack with both ipv4",
			ips:    []string{"1.2.3.4", "5.6.7.8"},
			policy: v1.IPFamilyPolicyPreferDualStack,
			family: PreferDualStack,
		},
		{
			desc:   "prefer dual stack with both ipv6",
			ips:    []string{"100::1", "100::2"},
			policy: v1.IPFamilyPolicyPreferDualStack,
			family: PreferDualStack,
		},
		{
			desc:    "prefer dual stack with empty address",
			ips:     []string{"", ""},
			policy:  v1.IPFamilyPolicyPreferDualStack,
			family:  Unknown,
			wantErr: true,
