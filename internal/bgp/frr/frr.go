...
func (sm *sessionManager) createConfig() (*frrConfig, error) {
	...
	for _, s := range sm.sessions {
		...
		for _, adv := range s.advertised {
			...
			for _, c := range adv.Communities {
				...
				prefixList, ok := neighbor.PrefixesModifiers[prefixListName]
				if !ok {
					prefixList = PropertyPrefixList{
						Name:         prefixListName,
						IPFamily:     frrFamily,
						SetStatement: setStatementForCommunity(c),
						prefixesSet:  sets.New[string](),
						Prefixes:     []string{},
					}
				}
				prefixList.prefixesSet.Insert(prefix)
				neighbor.PrefixesModifiers[prefixListName] = prefixList
			}
			if adv.LocalPref != 0 {
				...
				if !ok {
					prefixList = PropertyPrefixList{
						Name:         prefixListName,
						IPFamily:     frrFamily,
						SetStatement: setStatementForLocalPref(adv.LocalPref),
						prefixesSet:  sets.New[string](),
						Prefixes:     []string{},
					}
				}
	...
	for _, r := range sortMap(routers) {
		for _, n := range r.neighbors {
			...
			for k, m := range n.PrefixesModifiers {
				m.Prefixes = sets.List(n.PrefixesModifiers[k].prefixesSet)
				n.PrefixesModifiers[k] = m
			}
		}
		toAdd := &routerConfig{
			MyASN:        r.myASN,
...
func setStatementForCommunity(comm community.BGPCommunity) string {
	if community.IsLarge(comm) {
		return fmt.Sprintf("set large-community %s additive", comm.String())
	}
	return fmt.Sprintf("set community %s additive", comm.String())
}

func setStatementForLocalPref(localPref uint32) string {
	return fmt.Sprintf("set local-preference %d", localPref)
}
