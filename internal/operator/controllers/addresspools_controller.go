...
const (
	addrpools_configMapFile string = "./template/addrpools_config.yaml"
)

// AddressPoolsReconciler reconciles a AddressPools object
type AddressPoolsReconciler struct {
	client.Client
	Log    logr.Logger
...
func (r *AddressPoolsReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	log := r.Log.WithValues("addresspools", req.NamespacedName)
	log.Info("Reconciling AddressPools resource")

	instance := &metallbv1beta1.AddressPools{}
...
func (r *AddressPoolsReconciler) applyConfigMap(ctx context.Context, instance *metallbv1beta1.AddressPools) error {
...
func (r *AddressPoolsReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&metallbv1beta1.AddressPools{}).
