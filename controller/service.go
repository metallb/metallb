...
func (c *controller) convergeBalancer(l log.Logger, key string, svc *v1.Service) error {
	...
	if len(lbIPs) == 0 {
		...
		if len(lbIPs) == 1 && familyPolicy == v1.IPFamilyPolicyPreferDualStack {
			...
			var additionalFamily ipfamily.Family
			if existingFamily == ipfamily.IPv4 {
				additionalFamily = ipfamily.IPv6
			} else {
				additionalFamily = ipfamily.IPv4
			}
			desiredPool := valueForAnnotation(svc.Annotations, AnnotationIPAllocateFromPool, DeprecatedAnnotationIPAllocateFromPool)
			newIP, err := c.ips.AllocateFromPoolForFamily(key, svc, additionalFamily, desiredPool, k8salloc.Ports(svc), SharingKey(svc), k8salloc.BackendKey(svc))
			if err != nil {
