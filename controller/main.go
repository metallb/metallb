...
func main() {
	var (
		...
		controllerMetricsPort = flag.String("controller-metrics-port", os.Getenv("METALLB_CONTROLLER_METRICS_PORT"), "HTTP listening port for controller metrics")
	...
	_, present = os.LookupEnv("METALLB_CONTROLLER_METRICS_PORT")
	if !present {
		*controllerMetricsPort = "0"
	}
	...
	cfg := &k8s.Config{
		...
		MetricsPort:           *port,
		ControllerMetricsPort: *controllerMetricsPort,
