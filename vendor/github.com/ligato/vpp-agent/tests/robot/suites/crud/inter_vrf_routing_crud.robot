...
Start Two Agents And Then Configure With Default And Non Default VRF
    ...
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}    IP Fib Table 1 On agent_vpp_1 Should Contain Route With IP ${IP_5}/32
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${IP_6}/32
...
Create Next Route For Inter Vrf Routing
    Create Route On agent_vpp_2 With IP ${NET3}/24 With Next Hop VRF 1 From Vrf Id 0 And Type 1
    Create Route On agent_vpp_2 With IP ${NET3}/24 With Next Hop VRF 1 From Vrf Id 2 And Type 1
    Create Route On agent_vpp_2 With IP ${NET1}/24 With Next Hop VRF 0 From Vrf Id 1 And Type 1
    Create Route On agent_vpp_2 With IP ${NET2}/24 With Next Hop VRF 2 From Vrf Id 1 And Type 1
...
Check Inter VRF Routing Again
    ...
    IP Fib Table 2 On agent_vpp_2 Should Contain Route With IP ${NET3}/24
    IP Fib Table 2 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:
    IP Fib Table 0 On agent_vpp_2 Should Contain Route With IP ${NET3}/24
    IP Fib Table 0 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:

    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${NET1}/24
    IP Fib Table 1 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:
    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${NET2}/24
    IP Fib Table 1 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:
...
Check State After Delete
    ...
    ${status1}=           Run Keyword And Return Status    IP Fib Table 2 On agent_vpp_2 Should Contain Route With IP ${NET3}/24
    Should Not Be True    ${status1}

    ${status2}=           Run Keyword And Return Status    IP Fib Table 0 On agent_vpp_2 Should Contain Route With IP ${NET3}/24
    Should Not Be True    ${status2}

    ${status3}=           Run Keyword And Return Status    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${NET1}/24
    Should Not Be True    ${status3}

    ${status4}=           Run Keyword And Return Status    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${NET2}/24
    Should Not Be True    ${status4}

    ${status5}=           Run Keyword And Return Status    IP Fib Table 1 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:
    Should Not Be True    ${status5}

Update Inter Vrf Route
    Create Route On agent_vpp_2 With IP ${NET1}/24 With Next Hop VRF 0 From Vrf Id 1 And Type 1
    Create Route On agent_vpp_2 With IP ${NET1}/24 With Next Hop VRF 2 From Vrf Id 1 And Type 1

Check Route After Update
    Show IP Fib On agent_vpp_2
    IP Fib Table 1 On agent_vpp_2 Should Contain Route With IP ${NET1}/24
    IP Fib Table 1 On agent_vpp_2 Should Contain Vrf unicast lookup in ipv4-VRF:2
