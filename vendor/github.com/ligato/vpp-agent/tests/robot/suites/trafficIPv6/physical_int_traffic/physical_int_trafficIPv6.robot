...
Setup Interfaces
    vpp_ctl: Put Physical Interface With IP    node=agent_vpp_1    name=GigabitEthernet0/9/0    ip=${IP_1}
#    vpp_ctl: Put Memif Interface With IP    node=agent_vpp_1    name=vpp1_memif1    mac=62:61:61:61:61:61    master=true    id=1    ip=${IP_1}
##    vpp_ctl: Put Veth Interface With IP    node=agent_vpp_1    name=vpp1_veth1    mac=12:11:11:11:11:11    peer=vpp1_veth2    ip=${IP_3}
...
##    vpp_ctl: Put VXLan Interface    node=agent_vpp_1    name=vpp1_vxlan1    src=${IP_1}    dst=${IP_2}    vni=5
...
##    vpp_ctl: Put TAP Interface With IP    node=agent_vpp_1    name=vpp1_tap1    mac=32:21:21:11:11:11    ip=30.30.1.1    host_if_name=linux_vpp1_tap1
    ...
    vpp_ctl: Put Physical Interface With IP    node=agent_vpp_2    name=GigabitEthernet0/a/0    ip=${IP_2}
#    vpp_ctl: Put Memif Interface With IP    node=agent_vpp_2    name=vpp2_memif1    mac=62:62:62:62:62:62    master=false    id=1    ip=${IP_2}
    vpp_ctl: Put Veth Interface With IP    node=agent_vpp_2    name=vpp2_veth1    mac=22:21:21:21:21:21    peer=vpp2_veth2    ip=${IP_4}
    ...
    vpp_ctl: Put VXLan Interface    node=agent_vpp_2    name=vpp2_vxlan1    src=${IP_2}    dst=${IP_1}    vni=5
    @{ints}=    Create List    vpp2_vxlan1    vpp2_afpacket1
    vpp_ctl: Put Bridge Domain    node=agent_vpp_2    name=vpp2_bd1    ints=${ints}
    vpp_ctl: Put Loopback Interface With IP    node=agent_vpp_2    name=vpp2_loop1    mac=22:21:21:11:11:11    ip=${IP_5}
    vpp_ctl: Put TAP Interface With IP    node=agent_vpp_2    name=vpp2_tap1    mac=32:22:22:11:11:11    ip=${IP_6}    host_if_name=linux_vpp2_tap1
...
#    Should Contain    ${out}    ${int}
    ...
    Should Contain    ${out}    ${int}
    ${int}=    vpp_ctl: Get Interface Internal Name    agent_vpp_1    GigabitEthernet0/9/0
    Should Contain    ${out}    ${int}
...
#    Should Contain    ${out}    ${int}
    ...
    Should Contain    ${out}    ${int}
    ${int}=    vpp_ctl: Get Interface Internal Name    agent_vpp_2    GigabitEthernet0/a/0
    Should Contain    ${out}    ${int}
...
#    vpp_ctl: Delete VPP Interface    node=agent_vpp_2    name=${DOCKER_PHYSICAL_INT_2_VPP_NAME}
    sleep    5
    vpp_term: Show Interfaces    agent_vpp_1
    vpp_term: Show Interfaces    agent_vpp_2
    Write To Machine    agent_vpp_1_term    show int addr
