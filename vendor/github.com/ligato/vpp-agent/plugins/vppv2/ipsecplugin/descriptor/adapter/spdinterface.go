...
type SPDInterfaceDescriptorAdapter struct {
	descriptor *SPDInterfaceDescriptor
}

func NewSPDInterfaceDescriptor(typedDescriptor *SPDInterfaceDescriptor) *KVDescriptor {
	adapter := &SPDInterfaceDescriptorAdapter{descriptor: typedDescriptor}
...
func (da *SPDInterfaceDescriptorAdapter) ValueComparator(key string, oldValue, newValue proto.Message) bool {
	typedOldValue, err1 := castSPDInterfaceValue(key, oldValue)
	typedNewValue, err2 := castSPDInterfaceValue(key, newValue)
...
func (da *SPDInterfaceDescriptorAdapter) Modify(key string, oldValue, newValue proto.Message, oldMetadata Metadata) (newMetadata Metadata, err error) {
	oldTypedValue, err := castSPDInterfaceValue(key, oldValue)
	if err != nil {
		return nil, err
	}
	newTypedValue, err := castSPDInterfaceValue(key, newValue)
	if err != nil {
		return nil, err
	}
	typedOldMetadata, err := castSPDInterfaceMetadata(key, oldMetadata)
	if err != nil {
		return nil, err
	}
...
func (da *SPDInterfaceDescriptorAdapter) ModifyWithRecreate(key string, oldValue, newValue proto.Message, metadata Metadata) bool {
	oldTypedValue, err := castSPDInterfaceValue(key, oldValue)
	if err != nil {
		return true
	}
...
func (da *SPDInterfaceDescriptorAdapter) Dump(correlate []KVWithMetadata) ([]KVWithMetadata, error) {
	var correlateWithType []SPDInterfaceKVWithMetadata
	for _, kvpair := range correlate {
		typedValue, err := castSPDInterfaceValue(kvpair.Key, kvpair.Value)
		if err != nil {
			continue
		}
		typedMetadata, err := castSPDInterfaceMetadata(kvpair.Key, kvpair.Metadata)
		if err != nil {
			continue
		}
		correlateWithType = append(correlateWithType,
			SPDInterfaceKVWithMetadata{
				Key:      kvpair.Key,
				Value:    typedValue,
...
func castSPDInterfaceMetadata(key string, metadata Metadata) (interface{}, error) {
	if metadata == nil {
		return nil, nil
	}
